/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package mas.common.util.helper;

/**
 *
 * @author Lewis
 */
public class UserMsg {
    public static final String WRONG_PASSWORD_ERROR = "The password you entered is incorrect!";
    public static final String LOGIN_SUCCESS_MSG = "You have logged in successfully!";
    public static final String LOGIN_OUT_MSG = "You have logged out successfully!";
    public static final String NO_SUCH_USERNAME_ERROR = "User does not exist!";
    public static final String EXIST_USERNAME_ERROR = "Username has already existed";
    public static final String NO_SUCH_ROLE_ERROR = "Role does not exist!";
    public static final String NO_PERMISSION_ERROR = "There is no permission assigned to the role";
    public static final String NO_SUCH_EMAIL_ERROR = "Email does not exist";
    public static final String NO_MESSAGE_ERROR = "User does not have messages";
    public static final String EXIST_ROLE_ERROR = "Role already exist";
    public static final String EXIST_PERMISSION_ERROR = "Permission already exist";
    public static final String NO_USER_EXIST_ERROR = "No user exist in database";
    public static final String EXIST_USER_ERROR = "User already exist";
    public static final String EXIST_USER_EMAIL_ERROR = "Email already exist";
}
